...
class ReaderViewModelTest {
    ...
    private val deleteBookDailyReadingUseCase = mockk<DeleteBookDailyReadingUseCase>()
    ...
    fun setup() {
        ...
        coEvery { deleteBookDailyReadingUseCase(any(), any()) } returns Result.success(Unit)
    ...
    fun `initial state is Loading then Error if parse fails`() = runTest {
        ...
        viewModel = ReaderViewModel(getBookByIdUseCase, getReadingProgressByIdUseCase, getBookDailyReadingsUseCase, updateReadingProgressUseCase, addReadingProgressUseCase, addDailyReadingUseCase, deleteBookDailyReadingUseCase, updateBookUseCase, parseEpubUseCase, settingsRepository, savedStateHandle)
    ...
    @Test
    fun `OnDeleteDailyReading deletes the open book day and keeps progress and the sheet`() = runTest {
        viewModel = loadReaderViewModel()
        viewModel.onEvent(ReaderEvent.OnToggleSessionsDialog)
        advanceUntilIdle()
        clearMocks(addDailyReadingUseCase, answers = false, recordedCalls = true)
        coEvery { addDailyReadingUseCase(any()) } returns Result.success(Unit)

        val before = viewModel.uiState.value as ReaderUiState.Success
        viewModel.onEvent(ReaderEvent.OnDeleteDailyReading(bookId = 99, date = "2026-09-23"))
        advanceUntilIdle()

        val after = viewModel.uiState.value as ReaderUiState.Success
        coVerify(exactly = 1) { deleteBookDailyReadingUseCase(1, "2026-09-23") }
        coVerify(exactly = 0) { addDailyReadingUseCase(any()) }
        coVerify(exactly = 0) { updateReadingProgressUseCase(any()) }
        assertThat(after.showSessionsDialog).isTrue()
        assertThat(after.readingProgress.lastChapterIndex).isEqualTo(before.readingProgress.lastChapterIndex)
        assertThat(after.readingProgress.scrollPosition).isEqualTo(before.readingProgress.scrollPosition)
    }

    @Test
    fun `OnDeleteDailyReading shows a snackbar when delete fails`() = runTest {
        coEvery { deleteBookDailyReadingUseCase(any(), any()) } returns Result.failure(
            JuguitoException(R.string.error_delete_reading_session)
        )
        viewModel = loadReaderViewModel()

        viewModel.effect.test {
            viewModel.onEvent(ReaderEvent.OnDeleteDailyReading(bookId = 1, date = "2026-09-23"))

            val effect = awaitItem() as UiEffect.ShowSnackbar
            val message = effect.message as UiText.StringResource
            assertThat(message.resId).isEqualTo(R.string.error_delete_reading_session)
        }
    }
}
