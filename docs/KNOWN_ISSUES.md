...
| Severidad | IDs |
|-----------|-----|
| **Crítico** | DATA-001, READER-002, READER-003 |
...
| **Estado** | Abierto |
| **Target** | v1.2.0 |
| **Área** | BookDAO, FolderDAO, GenreDAO |

`@Insert(onConflict = REPLACE)` = DELETE + INSERT. Con FK `ON DELETE CASCADE` en `book_folders` / `book_genres`, actualizar carpeta/género/libro **destruye relaciones**.

**Síntomas:** renombrar carpeta → libros pierden carpeta; update libro → pierde carpetas/géneros.

**Fix:** `@Insert` para create, `@Update` para modify. Ver [DATABASE.md](DATABASE.md).
...
| **Estado** | Abierto |
| **Target** | v1.2.0 |
| **Área** | UpdateReadingProgressUseCase |

Al cambiar estado automático (Pendiente→Leyendo, 100%→Finalizado), llama `bookRepository.saveBook(book.copy(status = …))` que usa REPLACE → CASCADE borra carpetas, géneros, etc.

**Archivo:** `domain/usecase/readingProgress/UpdateReadingProgressUseCase.kt:38-40`

**Fix:** `@Update` en BookDAO o use case dedicado `updateBookStatus(id, status)` sin REPLACE.
